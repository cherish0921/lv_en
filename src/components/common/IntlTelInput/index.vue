<template>
  <div class="intl-tel-input allow-dropdown">
    <div class="flag-container">
      <div class="selected-flag" @click="hideSubMenu = !hideSubMenu;showsubmenufun('yy')"
           :title="currentData.name + ': +' + currentData.dialCode">
        <div :class="'iti-flag ' + currentData.code"></div>
        <span class="iti-text">{{currentData.name}}</span>
        <img  class="iti-arrow" src="./arrow.png" alt="">
      </div>
      <ul class="country-list" v-show="!hideSubMenu">
        <li @click="currentCode = item.code;hideSubMenu = true;setCountry(item);" :key="index" v-for="(item,index) in frontCountriesArray"
        ...
        <li class="divider"></li>
        <li @click="currentCode = item.code;hideSubMenu = true;setCountry(item);" :key="index" v-for="(item,index) in countriesArray"
            :class="'country ' + (item.code == currentCode ? 'highlight' : '')">
          <div class="flag-box">
            <div :class="'iti-flag ' + item.code"></div>
          </div>
...
<script>
  export default (function () {
    ...
    return {
      ...
      methods: {
        showsubmenufun:function(){
         let div=   document.querySelectorAll('.intl-tel-input .selected-flag  .iti-arrow')[0]
